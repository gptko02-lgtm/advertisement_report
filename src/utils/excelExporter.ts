// Excel 파일 생성 및 내보내기

import ExcelJS from 'exceljs';
import { AdData } from './dataParser';
import { SummaryMetrics, KeywordPerformance, PlatformComparison } from './analyticsEngine';
import { Insight } from './insightGenerator';

/**
 * 단일 시트로 구성된 Excel 리포트 생성
 */
export async function generateExcelReport(
    rawData: AdData[],
    metrics: SummaryMetrics,
    keywords: KeywordPerformance[],
    platforms: PlatformComparison[],
    insights: Insight[]
): Promise<void> {
    // ExcelJS를 안전하게 접근 (Next.js 빌드 호환성)
    const ExcelJSWorkbook = ((ExcelJS as any).default || ExcelJS).Workbook || ExcelJS.Workbook;
    const workbook = new ExcelJSWorkbook();

    // 단일 통합 시트 생성
    await createUnifiedSheet(workbook, rawData, metrics, keywords, platforms, insights);

    // 파일 다운로드
    const buffer = await workbook.xlsx.writeBuffer();
    downloadExcelFile(buffer, `ChatGPT교육_광고리포트_${new Date().toISOString().split('T')[0]}.xlsx`);
}

/**
 * 통합 단일 시트 생성
 */
async function createUnifiedSheet(
    workbook: ExcelJS.Workbook,
    rawData: AdData[],
    metrics: SummaryMetrics,
    keywords: KeywordPerformance[],
    platforms: PlatformComparison[],
    insights: Insight[]
) {
    const sheet = workbook.addWorksheet('광고 성과 리포트');

    let currentRow = 1;

    // ============================================
    // 제목
    // ============================================
    sheet.mergeCells(`A${currentRow}:H${currentRow}`);
    const titleCell = sheet.getCell(`A${currentRow}`);
    titleCell.value = '📊 ChatGPT 교육 광고 - 주간 성과 리포트';
    titleCell.font = { bold: true, size: 16, color: { argb: 'FFFFFFFF' } };
    titleCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF4472C4' } };
    titleCell.alignment = { horizontal: 'center', vertical: 'middle' };
    sheet.getRow(currentRow).height = 30;
    currentRow++;

    sheet.mergeCells(`A${currentRow}:H${currentRow}`);
    const dateCell = sheet.getCell(`A${currentRow}`);
    dateCell.value = `보고 기준일: ${new Date().toLocaleDateString('ko-KR')}`;
    dateCell.font = { size: 10 };
    dateCell.alignment = { horizontal: 'center', vertical: 'middle' };
    currentRow++;

    // ============================================
    // [1] 주요 지표 요약
    // ============================================
    currentRow = addSectionSpacer(sheet, currentRow);
    currentRow = addSectionHeader(sheet, currentRow, '📈 [1] 주요 지표 요약', 'FF4472C4');

    // 날짜 계산
    const today = new Date();
    const yesterday = new Date(today);
    yesterday.setDate(today.getDate() - 1);
    const dayBeforeYesterday = new Date(today);
    dayBeforeYesterday.setDate(today.getDate() - 2);

    const formatDate = (date: Date) => {
        const month = String(date.getMonth() + 1).padStart(2, '0');
        const day = String(date.getDate()).padStart(2, '0');
        return `${month}월 ${day}일`;
    };

    // 지표 테이블 헤더
    const metricsHeaderRow = sheet.addRow([
        '구분',
        formatDate(yesterday),
        formatDate(dayBeforeYesterday),
        '증감',
        '최근 7일',
        '이전 7일',
        '증감율',
        '당월'
    ]);
    styleHeaderRow(metricsHeaderRow, 'FF4472C4');
    currentRow++;

    // 지표 데이터
    const metricsData = [
        ['광고비', metrics.totalAdSpend, metrics.prevDayAdSpend, metrics.adSpendChange.toFixed(1) + '%', metrics.last7DaysAdSpend, metrics.prev7DaysAdSpend, metrics.adSpend7DayChange.toFixed(1) + '%', metrics.currentMonthAdSpend],
        ['CPC', Math.round(metrics.avgCPC), Math.round(metrics.prevDayAvgCPC), metrics.cpcChange.toFixed(1) + '%', Math.round(metrics.last7DaysAvgCPC), Math.round(metrics.prev7DaysAvgCPC), metrics.cpc7DayChange.toFixed(1) + '%', Math.round(metrics.currentMonthAvgCPC)],
        ['클릭수', metrics.totalClicks, metrics.prevDayClicks, '', metrics.last7DaysClicks, metrics.prev7DaysClicks, '', metrics.currentMonthClicks],
        ['CTR', metrics.avgCTR.toFixed(2) + '%', '', '', '', '', '', ''],
        ['노출수', metrics.totalImpressions, '', '', '', '', '', ''],
    ];

    metricsData.forEach(row => {
        const dataRow = sheet.addRow(row);
        styleDataRow(dataRow);

        // 숫자 셀에 쉼표 포맷 적용
        dataRow.eachCell((cell, colNumber) => {
            if (colNumber >= 2 && colNumber <= 8 && typeof cell.value === 'number') {
                cell.numFmt = '#,##0';
            }

            // 증감률 셀 색상
            if (colNumber === 4 || colNumber === 7) {
                const value = String(cell.value || '');
                if (value.includes('-')) {
                    cell.font = { color: { argb: 'FF00B050' } };
                } else if (value !== '' && !value.includes('0.0%')) {
                    cell.font = { color: { argb: 'FFFF0000' } };
                }
            }
        });
        currentRow++;
    });

    // 주요 인사이트
    currentRow++;
    const insightHeaderRow = sheet.addRow(['💡 주요 인사이트 (Top 5)']);
    sheet.mergeCells(`A${currentRow}:H${currentRow}`);
    const insightHeader = sheet.getCell(`A${currentRow}`);
    insightHeader.font = { bold: true, size: 11 };
    insightHeader.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF3F4F6' } };
    currentRow++;

    insights.slice(0, 5).forEach(insight => {
        let icon = '';
        let color = 'FF000000';

        if (insight.level === '즉시조치') {
            icon = '🔴';
            color = 'FFFF0000';
        } else if (insight.level === '적극적기회') {
            icon = '🟡';
            color = 'FFFFA500';
        } else {
            icon = '🟢';
            color = 'FF00B050';
        }

        const row = sheet.addRow([icon, insight.이유]);
        sheet.mergeCells(`B${currentRow}:H${currentRow}`);
        row.getCell(1).font = { color: { argb: color }, bold: true };
        row.getCell(2).alignment = { wrapText: true };
        styleDataRow(row);
        currentRow++;
    });

    // ============================================
    // [2] 매체별 비교
    // ============================================
    currentRow = addSectionSpacer(sheet, currentRow);
    currentRow = addSectionHeader(sheet, currentRow, '📊 [2] 매체별 비교 (Google vs Naver)', 'FF10B981');

    const platformHeaderRow = sheet.addRow(['매체', '최근7일 광고비', '점유율', '평균 CPC', 'CTR', '클릭수', '평가']);
    styleHeaderRow(platformHeaderRow, 'FF10B981');
    currentRow++;

    platforms.forEach(platform => {
        let evaluation = '';
        if (platform.avgCPC > 1000) {
            evaluation = 'CPC 효율성 개선 필요';
        } else if (platform.ctr < 0.1) {
            evaluation = 'CPC 우수, CTR 개선 필요';
        } else {
            evaluation = '안정적';
        }

        const dataRow = sheet.addRow([
            platform.platform,
            platform.adSpend,
            platform.점유율.toFixed(1) + '%',
            Math.round(platform.avgCPC),
            platform.ctr.toFixed(2) + '%',
            platform.clicks,
            evaluation,
        ]);

        styleDataRow(dataRow);

        // 숫자 셀에 쉼표 포맷 적용
        dataRow.getCell(2).numFmt = '#,##0';  // 광고비
        dataRow.getCell(4).numFmt = '#,##0';  // CPC
        dataRow.getCell(6).numFmt = '#,##0';  // 클릭수

        currentRow++;
    });

    // ============================================
    // [3] 키워드별 상세 분석
    // ============================================
    currentRow = addSectionSpacer(sheet, currentRow);
    currentRow = addSectionHeader(sheet, currentRow, '🔑 [3] 키워드별 상세 분석 (Top 12)', 'FFF59E0B');

    const keywordHeaderRow = sheet.addRow(['순위', '매체', '키워드', '최근7일 광고비', '이전7일 광고비', '증감', '최근7일 CPC', 'CTR', '성과']);
    styleHeaderRow(keywordHeaderRow, 'FFF59E0B');
    currentRow++;

    keywords.slice(0, 12).forEach((kw, index) => {
        const change = kw.최근7일광고비 - kw.이전7일광고비;
        const dataRow = sheet.addRow([
            index + 1,
            kw.매체,
            kw.키워드,
            kw.최근7일광고비,
            kw.이전7일광고비,
            change,
            kw.최근7일CPC,
            kw.CTR,
            kw.증감,
        ]);

        styleDataRow(dataRow);

        // 숫자 셀에 쉼표 포맷 적용
        dataRow.getCell(4).numFmt = '#,##0';  // 최근7일 광고비
        dataRow.getCell(5).numFmt = '#,##0';  // 이전7일 광고비
        dataRow.getCell(6).numFmt = '#,##0';  // 증감
        dataRow.getCell(7).numFmt = '#,##0';  // CPC

        // 증감 색상
        if (change > 0) {
            dataRow.getCell(6).font = { color: { argb: 'FFFF0000' } };
        } else if (change < 0) {
            dataRow.getCell(6).font = { color: { argb: 'FF00B050' } };
        }

        currentRow++;
    });

    // 성과 판단 기준
    currentRow++;
    const criteriaTitle = sheet.addRow(['📊 성과 판단 기준']);
    sheet.mergeCells(`A${currentRow}:I${currentRow}`);
    sheet.getCell(`A${currentRow}`).font = { bold: true, size: 11, color: { argb: 'FFF59E0B' } };
    currentRow++;

    const criteria = [
        ['🆕 신규:', '조건 1: 이전 7일 광고비가 0원이고, 최근 7일 광고비가 0원 초과'],
        ['', '조건 2: 이전 7일 광고비가 있지만 증감률이 -10% ~ +10% 사이 (유지)'],
        ['📈 증가:', '이전 7일 광고비 대비 +10% 초과 증가'],
        ['📉 감소:', '이전 7일 광고비 대비 -10% 미만 감소'],
        ['⏸️ 중단:', '최근 7일 광고비가 0원'],
    ];

    criteria.forEach(([label, desc]) => {
        const row = sheet.addRow([label, desc]);
        sheet.mergeCells(`B${currentRow}:I${currentRow}`);
        if (label) {
            row.getCell(1).font = { bold: true, size: 10 };
        }
        row.getCell(2).font = { size: 9 };
        row.getCell(2).alignment = { wrapText: true };
        currentRow++;
    });

    // ============================================
    // [4] 개선 제안 및 액션 플랜
    // ============================================
    currentRow = addSectionSpacer(sheet, currentRow);
    currentRow = addSectionHeader(sheet, currentRow, '🎯 [4] 개선 제안 및 액션 플랜', 'FFEF4444');

    // 즉시 조치 필요
    const urgentInsights = insights.filter(i => i.level === '즉시조치');
    if (urgentInsights.length > 0) {
        currentRow++;
        const urgentTitle = sheet.addRow(['🔴 즉시 조치 필요']);
        sheet.mergeCells(`A${currentRow}:E${currentRow}`);
        sheet.getCell(`A${currentRow}`).font = { bold: true, size: 12, color: { argb: 'FFFF0000' } };
        currentRow++;

        const urgentHeader = sheet.addRow(['No', '이유', '제안 액션', '기간', '우선순위']);
        styleHeaderRow(urgentHeader, 'FFFF6B6B');
        currentRow++;

        urgentInsights.forEach((insight, index) => {
            const row = sheet.addRow([
                index + 1,
                insight.이유,
                insight.제안액션,
                insight.기간,
                insight.우선순위,
            ]);
            styleDataRow(row);
            row.eachCell((cell) => {
                cell.alignment = { wrapText: true, vertical: 'top' };
            });
            currentRow++;
        });
    }

    // 적극적 기회
    const opportunityInsights = insights.filter(i => i.level === '적극적기회');
    if (opportunityInsights.length > 0) {
        currentRow++;
        const opportunityTitle = sheet.addRow(['🟡 적극적 기회']);
        sheet.mergeCells(`A${currentRow}:E${currentRow}`);
        sheet.getCell(`A${currentRow}`).font = { bold: true, size: 12, color: { argb: 'FFFFA500' } };
        currentRow++;

        const opportunityHeader = sheet.addRow(['No', '이유', '제안 액션', '기간', '우선순위']);
        styleHeaderRow(opportunityHeader, 'FFFFCC99');
        currentRow++;

        opportunityInsights.forEach((insight, index) => {
            const row = sheet.addRow([
                index + 1,
                insight.이유,
                insight.제안액션,
                insight.기간,
                insight.우선순위,
            ]);
            styleDataRow(row);
            row.eachCell((cell) => {
                cell.alignment = { wrapText: true, vertical: 'top' };
            });
            currentRow++;
        });
    }

    // 긍정적 지표
    const positiveInsights = insights.filter(i => i.level === '긍정적지표');
    if (positiveInsights.length > 0) {
        currentRow++;
        const positiveTitle = sheet.addRow(['🟢 긍정적 지표 (유지 전략)']);
        sheet.mergeCells(`A${currentRow}:E${currentRow}`);
        sheet.getCell(`A${currentRow}`).font = { bold: true, size: 12, color: { argb: 'FF00B050' } };
        currentRow++;

        const positiveHeader = sheet.addRow(['No', '이유', '제안 액션', '기간', '우선순위']);
        styleHeaderRow(positiveHeader, 'FF92D050');
        currentRow++;

        positiveInsights.forEach((insight, index) => {
            const row = sheet.addRow([
                index + 1,
                insight.이유,
                insight.제안액션,
                insight.기간,
                insight.우선순위,
            ]);
            styleDataRow(row);
            row.eachCell((cell) => {
                cell.alignment = { wrapText: true, vertical: 'top' };
            });
            currentRow++;
        });
    }

    // ============================================
    // [5] 일일 광고 성과 상세 데이터
    // ============================================
    currentRow = addSectionSpacer(sheet, currentRow);
    currentRow = addSectionHeader(sheet, currentRow, '📋 [5] 일일 광고 성과 상세 데이터', 'FF6366F1');

    const dailyHeaderRow = sheet.addRow(['매체', '키워드', '당일 광고비', '당일 CPC', '노출수', '클릭수', 'CTR', '캠페인']);
    styleHeaderRow(dailyHeaderRow, 'FF6366F1');
    currentRow++;

    rawData.forEach(row => {
        const dataRow = sheet.addRow([
            row.매체,
            row.키워드,
            row.당일광고비,
            row.당일CPC,
            row.노출수,
            row.클릭수,
            row.CTR,
            row.캠페인,
        ]);

        styleDataRow(dataRow);

        // 숫자 셀에 쉼표 포맷 적용
        dataRow.getCell(3).numFmt = '#,##0';  // 당일 광고비
        dataRow.getCell(4).numFmt = '#,##0';  // 당일 CPC
        dataRow.getCell(5).numFmt = '#,##0';  // 노출수
        dataRow.getCell(6).numFmt = '#,##0';  // 클릭수

        currentRow++;
    });

    // 열 너비 설정
    sheet.getColumn(1).width = 12;
    sheet.getColumn(2).width = 25;
    sheet.getColumn(3).width = 15;
    sheet.getColumn(4).width = 15;
    sheet.getColumn(5).width = 12;
    sheet.getColumn(6).width = 12;
    sheet.getColumn(7).width = 12;
    sheet.getColumn(8).width = 30;
    sheet.getColumn(9).width = 12;
}

/**
 * 섹션 간 공백 추가
 */
function addSectionSpacer(sheet: ExcelJS.Worksheet, currentRow: number): number {
    sheet.addRow([]);
    sheet.addRow([]);
    return currentRow + 2;
}

/**
 * 섹션 헤더 추가
 */
function addSectionHeader(sheet: ExcelJS.Worksheet, currentRow: number, title: string, color: string): number {
    sheet.mergeCells(`A${currentRow}:H${currentRow}`);
    const headerCell = sheet.getCell(`A${currentRow}`);
    headerCell.value = title;
    headerCell.font = { bold: true, size: 13, color: { argb: 'FFFFFFFF' } };
    headerCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: color } };
    headerCell.alignment = { horizontal: 'left', vertical: 'middle' };
    sheet.getRow(currentRow).height = 25;
    return currentRow + 1;
}

/**
 * 헤더 행 스타일 적용
 */
function styleHeaderRow(row: ExcelJS.Row, color: string) {
    row.eachCell((cell) => {
        cell.font = { bold: true, color: { argb: 'FFFFFFFF' } };
        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: color } };
        cell.alignment = { horizontal: 'center', vertical: 'middle' };
        cell.border = {
            top: { style: 'thin' },
            bottom: { style: 'thin' },
            left: { style: 'thin' },
            right: { style: 'thin' },
        };
    });
}

/**
 * 데이터 행 스타일 적용
 */
function styleDataRow(row: ExcelJS.Row) {
    row.eachCell((cell) => {
        cell.border = {
            top: { style: 'thin' },
            bottom: { style: 'thin' },
            left: { style: 'thin' },
            right: { style: 'thin' },
        };
    });
}

/**
 * Excel 파일 다운로드
 */
function downloadExcelFile(buffer: any, filename: string) {
    // Buffer를 Uint8Array로 변환하여 Blob으로 생성
    const uint8Array = new Uint8Array(buffer);
    const blob = new Blob([uint8Array], {
        type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
    });

    // Blob URL 생성 및 다운로드
    const url = window.URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = filename;

    // 링크를 DOM에 추가하고 클릭
    document.body.appendChild(link);
    link.click();

    // 정리
    document.body.removeChild(link);
    window.URL.revokeObjectURL(url);
}
